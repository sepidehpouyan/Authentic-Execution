...
void handle_output(void *session, char *data, uint16_t output_id, uint32_t param_types,
					 TEE_Param params[4]){

	const uint32_t exp_param_types = TEE_PARAM_TYPES(TEE_PARAM_TYPE_VALUE_OUTPUT,
	...
	encrypt_buf = TEE_Malloc(num*16, 0);
	...
	for(int i=0; i<num; i++){
		...
		encrypt = TEE_Malloc(16, 0);
		tag = TEE_Malloc(16, 0);
		if (!encrypt || !tag)
			goto out;
    
    	const void *text = data;
		TEE_Result res = TEE_AEEncryptFinal(sess->op_handle, text,sz, 
				encrypt,&sz, tag, &sz);
	
		if (!res) {
			printf("^^^^^winnnnnnnn^^^^^^\n");
			char *temp_encrypt = encrypt;
			char *temp_tag = tag;
			
      		for (int j = 0; j < 16; j++){
		   		printf("%02X", temp_encrypt[j]);
			}
			printf("\n");
			printf("********tag*********\n");

			for (int j = 0; j < 16; j++){
		   		printf("%02X", temp_tag[j]);
			}

			
			memcpy(encrypt_buf + (16*i), encrypt, 16);
...
void output(void *session, char *data, uint32_t param_types, TEE_Param params[4]){
	...
	struct aes_cipher *sess;
	sess = (struct aes_cipher *)session;
	uint16_t output_id = 6;
	handle_output(sess, data, output_id, param_types, params);
}
...
TEE_Result entry(void *session, uint32_t param_types,
	...
	char data[16] = {0};
	data[0]= 0x01;
	output(sess, data, param_types, params);
	return TEE_SUCCESS;
}
